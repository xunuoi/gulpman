...
function convertSource(rawSource, rawType, tarType){

    var repReg = new RegExp('\\.'+rawType+'$', 'g'),
        rsList = []

    if(!rawSource.match(repReg)) {
        rsList.push(rawSource)

        return rsList
    }

    if(typeof tarType == 'string'){
        rsList.push(rawSource.replace(repReg, '.'+tarType))
    }else {
        tarType.forEach((e, i)=>{
           rsList.push(rawSource.replace(repReg, '.'+e)) 
        })
    }
