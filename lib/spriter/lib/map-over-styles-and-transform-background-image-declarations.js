	...
	transformedStyles.stylesheet.rules.map(function(rule, ruleIndex) {
		if(rule.type === 'rule') {
			...
			rule.declarations = transformMap(rule.declarations, function(declaration, declarationIndex, declarations) {
				...
				// background-image always has a url且
				// 判断url是否有?__spriter后缀
				var gm_sprite_reg = /\?_gm_sprite/i,
					gm_sprite_str = '?_gm_sprite';

				if (transformedDeclaration.property === 'background-image' && gm_sprite_reg.test(transformedDeclaration.value)) {

					transformedDeclaration.value = transformedDeclaration.value.replace(gm_sprite_str, '');
					
					return cb(transformedDeclaration, declarationIndex, declarations);
				}
				//Background is a short hand property so make sure `url()` is in there
				// 且判断url是否有?__spriter后缀
				else if(transformedDeclaration.property === 'background' && gm_sprite_reg.test(transformedDeclaration.value)) {
